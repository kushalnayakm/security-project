...
// 5 frames × 400ms = 2 seconds stable before auto-capture
...
export default function MobileSession() {
  ...
  // ── doCapture: reads videoRef directly — never stale ─────────────────────
  const doCapture = useCallback(() => {
    const video = videoRef.current
    if (!video || video.readyState < 2 || video.videoWidth === 0) {
      console.warn('[doCapture] video not ready:', {
        exists: !!video,
        readyState: video?.readyState,
        width: video?.videoWidth,
      })
      // Retry once after 200ms in case video just needs a moment
      setTimeout(() => {
        ...
        console.log('[doCapture] ✓ retry succeeded, size:', dataUrl.length)
    ...
    canvas.width = w; canvas.height = h
    const ctx = canvas.getContext('2d')
    if (!ctx) { console.warn('[doCapture] no canvas context'); return }
    ...
    const dataUrl = canvas.toDataURL('image/jpeg', 0.92)
    console.log('[doCapture] ✓ captured, size:', dataUrl.length)
    setCapturedImage(dataUrl)
    setCameraActive(false)
    setScanStage('detected')
    setCountdown(null)
  ...
  // NO cameraReadyRef guard — that was causing the race condition.
  // We check videoRef.current directly instead.
  const handleValidationChange = useCallback((v: FaceValidation, loaded: boolean) => {
    setFaceValidation(v)
    if (loaded) setFaceModelsLoaded(true)

    console.log(
      '[validation] faceReady:', v.faceReady,
      '| frames:', stableFramesRef.current,
      '| captured:', autoCapturedRef.current,
      '| videoEl:', !!videoRef.current,
    )

    if (v.faceReady && !autoCapturedRef.current && videoRef.current) {
      stableFramesRef.current += 1

      // Show countdown: 2 → 1 as frames accumulate
      const framesLeft = STABLE_FRAMES_REQUIRED - stableFramesRef.current
      if (framesLeft > 0) {
        setCountdown(Math.ceil(framesLeft / 2))
      }

      if (stableFramesRef.current >= STABLE_FRAMES_REQUIRED) {
        console.log('[validation] ✓ STABLE — firing doCapture now')
  ...
  const handleContinueAfterFace = async () => {
    ...
    try {
      setBusy(true); setError(null); setScanStage('verifying')
  ...
  const handleRegisterFingerprint = () => {
    ...
    setFpStage('scanning'); setFpProgress(0); setError(null)
  ...
  const handleNameContinue = () => { if (name.trim().length < 2) return; setError(null); setUiStep('phone') }
  ...
  const handleSendOtp = async () => {
    ...
    try {
      ...
      const res = isLoginFlow
        ? await sendLoginOtp(sessionToken, countryCode, phoneNumber)
        : (() => { if (!facePhotoUrl) throw new Error('Missing face photo.'); return sendRegistrationOtp({ sessionToken, name: name.trim(), countryCode, phoneNumber, facePhotoUrl, biometricSupported, biometricRegistered }) })()
      const resolved = await res
      setSession(resolved.session)
      setDebugOtp(resolved.otp)
      setUiStep('otp')
    } catch (err) {
        ...
        {uiStep === 'face' && (
          <StepShell title="Face Capture">
            <div className="space-y-3">
              <div className={`relative overflow-hidden rounded-[24px] border-2 bg-slate-950 transition-colors duration-300 ${
                ...
                <div className="mx-auto aspect-[4/5] max-h-[44dvh] min-h-[280px] w-full max-w-[320px] bg-slate-900 sm:max-h-[48dvh]">
                  ...
                  <CameraCapture
                    ...
                    onVideoReady={(v) => {
                      // Write to ref — never causes stale closure in doCapture
                      videoRef.current = v
                      console.log('[VideoReady]', v ? `${v.videoWidth}×${v.videoHeight}` : 'null')
                    }}
                    onFaceCountChange={() => {}}
              ...
              <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
                {!cameraActive && !capturedImage && (
                  <Button fullWidth onClick={handleScanFace}>Scan Face</Button>
                )}
                {capturedImage && (
                  <>
                    ...
                    <Button fullWidth onClick={() => void handleContinueAfterFace()} loading={busy}>
                      Continue
                    </Button>
                  </>
                )}
        ...
        {uiStep === 'biometric' && (
          <StepShell title={isLoginFlow ? 'Verify Biometric' : 'Register Biometric'}>
            <div className="space-y-4">
              ...
              {fpStage === 'failed'   && <Button fullWidth onClick={handleRegisterFingerprint}>Retry</Button>}
            </div>
          </StepShell>
        )}
